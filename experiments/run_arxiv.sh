...
for a in "${alphas[@]}"; do
for b in "${betas[@]}"; do
    python main.py --log info --data arxiv --model GPCANet --lr 0.005 --wd 0 --nlayer $layer --nhid $hid --dropout $drop --epochs 500 --seed $seed --act ReLU --alpha 1 --beta $b --powers 5 --gpu $gpu --powers 5
done 
done 
done
done
